...
export default function ChartScanner({ variant = "default", active = true }) {
  ...
  const managementPlan = useMemo(
    () => describeManagementPlan(tradeManagement, signal?.timeframe),
    [tradeManagement, signal?.timeframe]
  );
  const tpLadder = useMemo(() => {
    const multiples = tpRewardMultiples(signal?.timeframe || "M15");
    return {
      multiples,
      labels: multiples.map((m) => `1:${m}`),
      summary: tpRiskRewardLabel(signal?.timeframe || "M15"),
    };
  }, [signal?.timeframe]);
        ...
        <div className="cs-tp-config" aria-label="Take-profit risk reward ratios">
          ...
          <div className="cs-tp-config-row" role="group" aria-label="Fixed TP ratios">
            <div className="cs-tp-ratio">
              <span>TP1</span>
              <strong>{tpLadder.labels[0]}</strong>
            </div>
            <div className="cs-tp-ratio">
              <span>TP2</span>
              <strong>{tpLadder.labels[1]}</strong>
            </div>
            <div className="cs-tp-ratio">
              <span>TP3</span>
              <strong>{tpLadder.labels[2]}</strong>
            </div>
          </div>
